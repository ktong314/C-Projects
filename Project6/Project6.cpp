...
int minRec2(int x[], int n) {
    ...
    int smallest1 = minRec2(x, n/2);
    int smallest2 = minRec2(x + (n/2), n/2);
...
double sqrtRec(double x, double low_guess, double high_guess) {
    ...
    if ((mid * mid) - x < 0.00000000000001 && (mid * mid) - x > -0.00000000000001){
        return mid;
...
Martian change(int cents) {
    ...
    else if(cents >= 12 && cents%12 < cents/5) {
        cents -= 12;
        x = change(cents);
        x.dodeks++;
        return x;
...
Martian change(int cents, int nick_val, int dodek_val) {
    ...
    if(cents < nick_val){
        martian.pennies += cents;
        return martian;
    }
    else if(cents >= dodek_val && cents%dodek_val < cents/nick_val) {
        cents -= 12;
        martian = change(cents);
        martian.dodeks++;
        return martian;
    } else {
        cents -= 5;
